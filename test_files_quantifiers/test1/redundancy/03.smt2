(set-logic HO_ALL)
(set-option :produce-models true)
(set-option :finite-model-find true)
(set-option :check-models true)
(set-option :sets-exp true)
(set-option :dag-thresh 0)
(set-option :uf-lazy-ll true)
(set-option :fmf-bound true)
(set-option :tlimit-per 20000)
(set-option :produce-models true)
(set-option :finite-model-find true)
(set-option :check-models true)
(set-option :sets-exp true)
(set-option :dag-thresh 0)
(set-option :uf-lazy-ll true)
(set-option :fmf-bound true)
(set-option :tlimit-per 20000)
(set-option :produce-models true)
(set-option :finite-model-find true)
(set-option :check-models true)
(set-option :sets-exp true)
(set-option :dag-thresh 0)
(set-option :uf-lazy-ll true)
(set-option :fmf-bound true)
(set-option :tlimit-per 20000)
(set-option :produce-models true)
(set-option :finite-model-find true)
(set-option :check-models true)
(set-option :sets-exp true)
(set-option :dag-thresh 0)
(set-option :uf-lazy-ll true)
(set-option :fmf-bound true)
(set-option :tlimit-per 20000)
(declare-const time (Set (Tuple Int)))
(declare-const A (Set (Tuple Int)))
(declare-const not_A (Set (Tuple Int Int)))
(declare-const B (Set (Tuple Int)))
(declare-const not_B (Set (Tuple Int Int)))
(declare-const C (Set (Tuple Int)))
(declare-const not_C (Set (Tuple Int Int)))
(declare-const Measure (Set (Tuple Int Int)))
(assert (exists ((A_time_Int_55 Int)) (and (exists ((Measure_time_Int_56 Int) (Measure_m1_Int_57 Int)) (and (and (not (exists ((not_C_start_time_Int_59 Int) (not_C_end_time_Int_60 Int)) (and (and (= not_C_end_time_Int_60 (+ A_time_Int_55 11)) (= not_C_start_time_Int_59 (+ A_time_Int_55 0))) (set.member (tuple not_C_start_time_Int_59 not_C_end_time_Int_60) not_C)))) (= A_time_Int_55 Measure_time_Int_56)) (set.member (tuple Measure_time_Int_56 Measure_m1_Int_57) Measure))) (set.member (tuple A_time_Int_55) A))))
(assert (forall ((A_time_Int_1 Int)) (=> (set.member (tuple A_time_Int_1) A) (exists ((Measure_time_Int_2 Int) (Measure_m1_Int_3 Int)) (and (and (exists ((B_time_Int_4 Int)) (and (and (>= B_time_Int_4 (+ A_time_Int_1 0)) (<= B_time_Int_4 (+ A_time_Int_1 7))) (set.member (tuple B_time_Int_4) B))) (= A_time_Int_1 Measure_time_Int_2)) (set.member (tuple Measure_time_Int_2 Measure_m1_Int_3) Measure))))))
(assert (forall ((B_time_Int_15 Int)) (=> (set.member (tuple B_time_Int_15) B) (exists ((Measure_time_Int_16 Int) (Measure_m1_Int_17 Int)) (and (and (exists ((C_time_Int_18 Int)) (and (and (>= C_time_Int_18 (+ B_time_Int_15 0)) (<= C_time_Int_18 (+ B_time_Int_15 7))) (set.member (tuple C_time_Int_18) C))) (= B_time_Int_15 Measure_time_Int_16)) (set.member (tuple Measure_time_Int_16 Measure_m1_Int_17) Measure))))))
(assert (forall ((A_time_Int_29 Int)) (=> (set.member (tuple A_time_Int_29) A) (exists ((Measure_time_Int_30 Int) (Measure_m1_Int_31 Int)) (and (and (or (exists ((C_time_Int_32 Int)) (and (and (>= C_time_Int_32 (+ A_time_Int_29 0)) (<= C_time_Int_32 (+ A_time_Int_29 15))) (set.member (tuple C_time_Int_32) C))) (not (< Measure_m1_Int_31 20))) (= A_time_Int_29 Measure_time_Int_30)) (set.member (tuple Measure_time_Int_30 Measure_m1_Int_31) Measure))))))
(assert (and (and (= (as set.empty (Set (Tuple Int Int Int))) (set.filter (lambda ((tuple_3 (Tuple Int Int Int))) (not (not (and (<= ((_ tuple.select 0) tuple_3) ((_ tuple.select 2) tuple_3)) (<= ((_ tuple.select 1) tuple_3) ((_ tuple.select 0) tuple_3)))))) (rel.product C not_C))) (and (= (as set.empty (Set (Tuple Int Int Int))) (set.filter (lambda ((tuple_2 (Tuple Int Int Int))) (not (not (and (<= ((_ tuple.select 0) tuple_2) ((_ tuple.select 2) tuple_2)) (<= ((_ tuple.select 1) tuple_2) ((_ tuple.select 0) tuple_2)))))) (rel.product B not_B))) (= (as set.empty (Set (Tuple Int Int Int))) (set.filter (lambda ((tuple_1 (Tuple Int Int Int))) (not (not (and (<= ((_ tuple.select 0) tuple_1) ((_ tuple.select 2) tuple_1)) (<= ((_ tuple.select 1) tuple_1) ((_ tuple.select 0) tuple_1)))))) (rel.product A not_A))))) (forall ((Measure_time_Int_67 Int) (Measure_m1_Int_68 Int) (Measure_time_Int_69 Int) (Measure_m1_Int_70 Int)) (=> (and (set.member (tuple Measure_time_Int_69 Measure_m1_Int_70) Measure) (set.member (tuple Measure_time_Int_67 Measure_m1_Int_68) Measure)) (=> (= Measure_time_Int_67 Measure_time_Int_69) (and (= Measure_m1_Int_68 Measure_m1_Int_70) (= Measure_time_Int_67 Measure_time_Int_69)))))))
(assert (=> (exists ((A_time_Int_71 Int)) (and true (set.member (tuple A_time_Int_71) A))) (and (exists ((A_time_Int_74 Int)) (and (forall ((A_time_Int_75 Int)) (=> (set.member (tuple A_time_Int_75) A) (<= A_time_Int_75 A_time_Int_74))) (set.member (tuple A_time_Int_74) A))) (exists ((A_time_Int_72 Int)) (and (forall ((A_time_Int_73 Int)) (=> (set.member (tuple A_time_Int_73) A) (>= A_time_Int_73 A_time_Int_72))) (set.member (tuple A_time_Int_72) A))))))
(assert (=> (exists ((B_time_Int_76 Int)) (and true (set.member (tuple B_time_Int_76) B))) (and (exists ((B_time_Int_79 Int)) (and (forall ((B_time_Int_80 Int)) (=> (set.member (tuple B_time_Int_80) B) (<= B_time_Int_80 B_time_Int_79))) (set.member (tuple B_time_Int_79) B))) (exists ((B_time_Int_77 Int)) (and (forall ((B_time_Int_78 Int)) (=> (set.member (tuple B_time_Int_78) B) (>= B_time_Int_78 B_time_Int_77))) (set.member (tuple B_time_Int_77) B))))))
(assert (=> (exists ((C_time_Int_81 Int)) (and true (set.member (tuple C_time_Int_81) C))) (and (exists ((C_time_Int_84 Int)) (and (forall ((C_time_Int_85 Int)) (=> (set.member (tuple C_time_Int_85) C) (<= C_time_Int_85 C_time_Int_84))) (set.member (tuple C_time_Int_84) C))) (exists ((C_time_Int_82 Int)) (and (forall ((C_time_Int_83 Int)) (=> (set.member (tuple C_time_Int_83) C) (>= C_time_Int_83 C_time_Int_82))) (set.member (tuple C_time_Int_82) C))))))
(assert (forall ((time_val_Int_0 Int)) (=> (set.member (tuple time_val_Int_0) time) (>= time_val_Int_0 0))))
(assert true)
(assert true)
(assert true)
(assert true)
(assert true)
(assert true)
(assert true)
(assert (forall ((Measure_time_Int_86 Int) (Measure_m1_Int_87 Int)) (=> (set.member (tuple Measure_time_Int_86 Measure_m1_Int_87) Measure) (exists ((time_val_Int_88 Int)) (and (= time_val_Int_88 Measure_time_Int_86) (set.member (tuple time_val_Int_88) time))))))
(check-sat)
(get-model)
