...
options["save_to"] = "cactus.svg"
plotter = cactus.Cactus(options)
instances = len(filters.tolist())
accumulative_filter = np.cumsum(filters.tolist())
accumulative_quantifiers = np.cumsum(quantifiers.tolist())
data = [
    ["filter", accumulative_filter, instances, options["timeout"]],
    ["quantifiers", accumulative_quantifiers, instances, options["timeout"]],
]
