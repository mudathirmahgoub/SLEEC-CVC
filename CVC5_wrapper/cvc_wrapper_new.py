...
def exists_relation(kinds, term_func):
    return set_quantifier(kinds, term_func, Kind.SET_SOME)
...
def set_quantifier(kinds, term_func, quantifier_kind=Kind.SET_ALL):
    if isinstance(kinds, Relation) or \
            (isinstance(kinds, list) and kinds and isinstance(kinds[0], Relation)):
        if isinstance(kinds, Relation):
            ...
            lambda_func = tm.mkTerm(Kind.LAMBDA, variable_list, val(term))            
            formula = val(tm.mkTerm(quantifier_kind, lambda_func,
                          kinds.relation))
            return formula
        else:
            instances = []
...
if __name__ == "__main__":
    ...
    # filtering_constraint = set_quantifier(birth, lambda b: (b.parent > 5) & (b.child < 3))
    #
    # new_map_filter = set_quantifier([birth, people], lambda b, p: IMPLIES(b.parent > 5, p.person > (b.child + 3)))
    #
